...
class Task extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->lang->admin_load('task', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->admin_model('task_model');
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    $product_details = $this->task_model->getProductByCode($item_code);
                    if ($item_expiry) {
                        $today = date('Y-m-d');
        ...
        if ($this->form_validation->run() == true && $this->task_model->addTask($data, $products, $attachments)) {
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line('tasks_added'));
            admin_redirect('task');
        } else {
            if ($quote_id) {
                $this->data['quote'] = $this->task_model->getQuoteByID($quote_id);
                $supplier_id         = $this->data['quote']->supplier_id;
                $items               = $this->task_model->getAllQuoteItems($quote_id);
                krsort($items);
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            $crow->expiry         = '';
                            $options              = $this->task_model->getProductOptions($crow->id);
                        ...
                        $options             = $this->task_model->getProductOptions($row->id);
            ...
            $bc                 = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('task'), 'page' => lang('task')], ['link' => '#', 'page' => lang('add_task')]];
            $meta               = ['page_title' => lang('add_task'), 'bc' => $bc];
            $this->page_construct('task/add', $meta, $this->data);
        }
    }
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->task_model->addExpense($data, $attachments)) {
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['categories'] = $this->task_model->getExpenseCategories();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->load->view($this->theme . 'task/add_expense', $this->data);
        }
    }
    ...
    public function add_payment($id = null)
    {
        ...
        $tasks = $this->task_model->getTaskByID($id);
        if ($tasks->payment_status == 'paid' && $tasks->grand_total == $tasks->paid) {
            $this->session->set_flashdata('error', lang('tasks_already_paid'));
            $this->sma->md();
        }
        ...
        if ($this->form_validation->run() == true && $this->task_model->addPayment($payment)) {
            ...
            $this->load->view($this->theme . 'task/add_payment', $this->data);
        }
    }

    public function combine_pdf($task_id)
    {
        $this->sma->checkPermissions('pdf');

        foreach ($task_id as $tasks_id) {
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $inv                 = $this->task_model->getTaskByID($tasks_id);
            if (!$this->session->userdata('view_right')) {
                $this->sma->view_rights($inv->created_by);
            }
            $this->data['rows']            = $this->task_model->getAllTaskItems($tasks_id);
            $this->data['supplier']        = $this->site->getCompanyByID($inv->supplier_id);
            $this->data['warehouse']       = $this->site->getWarehouseByID($inv->warehouse_id);
            $this->data['created_by']      = $this->site->getUser($inv->created_by);
            $this->data['inv']             = $inv;
            $this->data['return_tasks'] = $inv->return_id ? $this->task_model->getTaskByID($inv->return_id) : null;
            $this->data['return_rows']     = $inv->return_id ? $this->task_model->getAllTaskItems($inv->return_id) : null;
            $inv_html                      = $this->load->view($this->theme . 'task/pdf', $this->data, true);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->task_model->deleteTask($id)) {
            if ($this->input->is_ajax_request()) {
                $this->sma->send_json(['error' => 0, 'msg' => lang('tasks_deleted')]);
            }
    ...
    public function delete_expense($id = null)
    {
        ...
        $expense = $this->task_model->getExpenseByID($id);
        if ($this->task_model->deleteExpense($id)) {
    ...
    public function delete_payment($id = null)
    {
        ...
        if ($this->task_model->deletePayment($id)) {
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->task_model->getTaskByID($id);
        if ($inv->status == 'returned' || $inv->return_id || $inv->return_tasks_ref) {
            $this->session->set_flashdata('error', lang('tasks_x_action'));
        ...
        if ($this->form_validation->run() == true && $this->task_model->updateTask($id, $data, $products, $attachments)) {
            $this->session->set_userdata('remove_pols', 1);
            $this->session->set_flashdata('message', $this->lang->line('tasks_added'));
            admin_redirect('task');
        } else {
            ...
            if ($this->Settings->disable_editing) {
                if ($this->data['inv']->date <= date('Y-m-d', strtotime('-' . $this->Settings->disable_editing . ' days'))) {
                    $this->session->set_flashdata('error', sprintf(lang('tasks_x_edited_older_than_x_days'), $this->Settings->disable_editing));
                    redirect($_SERVER['HTTP_REFERER']);
                }
            }
            $inv_items = $this->task_model->getAllTaskItems($id);
            // krsort($inv_items);
            $c = rand(100000, 9999999);
            ...
            $this->data['id']         = $id;
            $this->data['suppliers']  = $this->site->getAllCompanies('supplier');
            $this->data['tasks']   = $this->task_model->getTaskByID($id);
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates']  = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->load->helper('string');
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc                 = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('task'), 'page' => lang('task')], ['link' => '#', 'page' => lang('edit_tasks')]];
            $meta               = ['page_title' => lang('edit_tasks'), 'bc' => $bc];
            $this->page_construct('task/edit', $meta, $this->data);
        }
    }
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->task_model->updateExpense($id, $data, $attachments)) {
            $this->session->set_flashdata('message', lang('expense_updated'));
            admin_redirect('task/expenses');
        } else {
            $this->data['error']      = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['expense']    = $this->task_model->getExpenseByID($id);
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->data['categories'] = $this->task_model->getExpenseCategories();
            $this->load->view($this->theme . 'task/edit_expense', $this->data);
        }
    }
    ...
    public function edit_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->task_model->updatePayment($id, $payment)) {
            $this->session->set_flashdata('message', lang('payment_updated'));
            admin_redirect('task');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['payment']  = $this->task_model->getPaymentByID($id);
            $this->data['modal_js'] = $this->site->modal_js();

            $this->load->view($this->theme . 'task/edit_payment', $this->data);
        }
    }

    public function email($tasks_id = null)
    {
        $this->sma->checkPermissions(false, true);

        if ($this->input->get('id')) {
            $tasks_id = $this->input->get('id');
        }
        $inv = $this->task_model->getTaskByID($tasks_id);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attachment = $this->pdf($tasks_id, null, 'S');

            try {
                if ($this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
                    delete_files($attachment);
                    $this->db->update('task', ['status' => 'ordered'], ['id' => $tasks_id]);
            ...
            if (file_exists('./themes/' . $this->Settings->theme . '/admin/views/email_templates/tasks.html')) {
                $tasks_temp = file_get_contents('themes/' . $this->Settings->theme . '/admin/views/email_templates/tasks.html');
            } else {
                $tasks_temp = file_get_contents('./themes/default/admin/views/email_templates/tasks.html');
            }
            $this->data['subject'] = ['name' => 'subject',
                'id'                         => 'subject',
                'type'                       => 'text',
                'value'                      => $this->form_validation->set_value('subject', lang('tasks_order') . ' (' . $inv->reference_no . ') ' . lang('from') . ' ' . $this->Settings->site_name),
            ];
            $this->data['note'] = ['name' => 'note',
                'id'                      => 'note',
                'type'                    => 'text',
                'value'                   => $this->form_validation->set_value('note', $tasks_temp),
            ];
            $this->data['supplier'] = $this->site->getCompanyByID($inv->supplier_id);

            $this->data['id']       = $tasks_id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'task/email', $this->data);
        }
    }
    ...
    {
        $this->sma->checkPermissions('payments', true);
        $payment               = $this->task_model->getPaymentByID($id);
        $inv                   = $this->task_model->getTaskByID($payment->tasks_id);
        $supplier              = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['inv']     = $inv;
        ...
        $html                     = $this->load->view($this->theme . 'task/payment_note', $this->data, true);
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $this->task_model->deleteExpense($id);
                    }
    ...
    public function expense_note($id = null)
    {
        ...
        $this->data['category']    = $expense->category_id ? $this->task_model->getExpenseCategoryByID($expense->category_id) : null;
        ...
        $this->load->view($this->theme . 'task/expense_note', $this->data);
    }
    ...
    public function expenses($id = null)
    {
        ...
        $this->page_construct('task/expenses', $meta, $this->data);
    }
    ...
    public function getExpenses()
    {
        ...
        $detail_link = anchor('admin/task/expense_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('expense_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link   = anchor('admin/task/edit_expense/$1', '<i class="fa fa-edit"></i> ' . lang('edit_expense'), 'data-toggle="modal" data-target="#myModal"');
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('task/delete_expense/$1') . "'>"
    ...
    public function getTask($warehouse_id = null)
    {
        ...
        $detail_link      = anchor('admin/task/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('tasks_details'));
        $payments_link    = anchor('admin/task/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('admin/task/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $email_link       = anchor('admin/task/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_tasks'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link        = anchor('admin/task/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_tasks'));
        $pdf_link         = anchor('admin/task/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode    = anchor('admin/products/print_barcodes/?tasks=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
        $return_link      = anchor('admin/task/return_tasks/$1', '<i class="fa fa-angle-double-left"></i> ' . lang('return_tasks'));
        $delete_link      = "<a href='#' class='po' title='<b>" . $this->lang->line('delete_tasks') . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('task/delete/$1') . "'>"
        . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
        . lang('delete_tasks') . '</a>';
        $action = '<div class="text-center"><div class="btn-group text-left">'
        . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
    ...
    public function index($warehouse_id = null)
    {
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('task')]];
        $meta = ['page_title' => lang('task'), 'bc' => $bc];
        $this->page_construct('task/index', $meta, $this->data);
    }

    /* ----------------------------------------------------------------------------- */

    public function modal_view($tasks_id = null)
    {
        ...
        $this->data['warehouse']       = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv']             = $inv;
        $this->data['payments']        = $this->task_model->getPaymentsForTask($tasks_id);
        $this->data['created_by']      = $this->site->getUser($inv->created_by);
        $this->data['updated_by']      = $inv->updated_by ? $this->site->getUser($inv->updated_by) : null;
        $this->data['return_tasks'] = $inv->return_id ? $this->task_model->getTaskByID($inv->return_id) : null;
        $this->data['return_rows']     = $inv->return_id ? $this->task_model->getAllTaskItems($inv->return_id) : null;
        $this->data['attachments']     = $this->site->getAttachments($tasks_id, 'tasks');

        $this->load->view($this->theme . 'task/modal_view', $this->data);
    }
    ...
    public function payment_note($id = null)
    {
        ...
        $this->load->view($this->theme . 'task/payment_note', $this->data);
    }
    ...
    public function payments($id = null)
    {
        ...
        $this->data['payments'] = $this->task_model->getTaskPayments($id);
        $this->data['inv']      = $this->task_model->getTaskByID($id);
        $this->load->view($this->theme . 'task/payments', $this->data);
    }
    ...
    public function pdf($tasks_id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['return_tasks'] = $inv->return_id ? $this->task_model->getTaskByID($inv->return_id) : null;
        $this->data['return_rows']     = $inv->return_id ? $this->task_model->getAllTaskItems($inv->return_id) : null;
        $name                          = $this->lang->line('tasks') . '_' . str_replace('/', '_', $inv->reference_no) . '.pdf';
        $html                          = $this->load->view($this->theme . 'task/pdf', $this->data, true);
        if (!$this->Settings->barcode_img) {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->session->set_flashdata('message', $this->lang->line('task_deleted'));
                    redirect($_SERVER['HTTP_REFERER']);
                    ...
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('task'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        $tasks = $this->task_model->getTaskByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($tasks->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $tasks->reference_no);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $tasks->supplier);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $tasks->status);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->sma->formatMoney($tasks->grand_total));
                        $row++;
                    }
                    ...
                    $this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical('center');
                    $filename = 'task_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
                $this->session->set_flashdata('error', $this->lang->line('no_tasks_selected'));
                redirect($_SERVER['HTTP_REFERER']);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    public function tasks_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES['userfile'])) {
                ...
                if (!$this->upload->do_upload()) {
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    admin_redirect('task/tasks_by_csv');
                }
                ...
                foreach ($final as $csv_pr) {
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        if ($product_details = $this->task_model->getProductByCode($csv_pr['code'])) {
                            if ($csv_pr['variant']) {
                                $item_option = $this->task_model->getProductVariantByName($csv_pr['variant'], $product_details->id);
                                if (!$item_option) {
                            ...
                            $gst_data      = [];
                            $tax_details   = ((isset($item_tax_rate) && !empty($item_tax_rate)) ? $this->task_model->getTaxRateByName($item_tax_rate) : $this->site->getTaxRateByID($product_details->tax_rate));
                            if ($tax_details) {
                                $ctax     = $this->site->calculateTax($product_details, $tax_details, $unit_cost);
                                $item_tax = $this->sma->formatDecimal($ctax['amount']);
                                $tax      = $ctax['tax'];
        ...
        if ($this->form_validation->run() == true && $this->task_model->addTask($data, $products, $attachments)) {
            ...
            $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('task'), 'page' => lang('task')], ['link' => '#', 'page' => lang('add_tasks_by_csv')]];
            $meta = ['page_title' => lang('add_tasks_by_csv'), 'bc' => $bc];
            $this->page_construct('task/tasks_by_csv', $meta, $this->data);
        }
    }

    public function return_tasks($id = null)
    {
        $this->sma->checkPermissions('return_task');
        ...
        $tasks = $this->task_model->getTaskByID($id);
        if ($tasks->return_id) {
            $this->session->set_flashdata('error', lang('tasks_already_returned'));
            redirect($_SERVER['HTTP_REFERER']);
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $note             = $this->sma->clear_tags($this->input->post('note'));
            $supplier_details = $this->site->getCompanyByID($tasks->supplier_id);
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_code          = $_POST['product'][$r];
                $tasks_item_id   = $_POST['tasks_item_id'][$r];
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $product = [
                        ...
                        'warehouse_id'      => $tasks->warehouse_id,
                        'item_tax'          => $pr_item_tax,
                        'tax_rate_id'       => $item_tax_rate,
                        'tax'               => $tax,
                        'discount'          => $item_discount,
            ...
            $data           = ['date' => $date,
                'tasks_id'         => $id,
                'reference_no'        => $tasks->reference_no,
                'supplier_id'         => $tasks->supplier_id,
                'supplier'            => $tasks->supplier,
                'warehouse_id'        => $tasks->warehouse_id,
                'note'                => $note,
                'total'               => $total,
                'product_discount'    => $product_discount,
                ...
                'grand_total'         => $grand_total,
                'created_by'          => $this->session->userdata('user_id'),
                'return_tasks_ref' => $reference,
                'status'              => 'returned',
                'payment_status'      => $tasks->payment_status == 'paid' ? 'due' : 'pending',
            ];
            if ($this->Settings->indian_gst) {
                $data['cgst'] = $total_cgst;
                $data['sgst'] = $total_sgst;
        ...
        if ($this->form_validation->run() == true && $this->task_model->addTask($data, $products, $attachments)) {
            $this->session->set_flashdata('message', lang('return_tasks_added'));
            admin_redirect('task');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            if ($this->data['inv']->status != 'received' && $this->data['inv']->status != 'partial') {
                $this->session->set_flashdata('error', lang('tasks_status_x_received'));
                redirect($_SERVER['HTTP_REFERER']);
            }
            if ($this->Settings->disable_editing) {
                if ($this->data['inv']->date <= date('Y-m-d', strtotime('-' . $this->Settings->disable_editing . ' days'))) {
            ...
            $this->data['reference'] = '';
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $bc                      = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('task'), 'page' => lang('task')], ['link' => '#', 'page' => lang('return_tasks')]];
            $meta                    = ['page_title' => lang('return_tasks'), 'bc' => $bc];
            $this->page_construct('task/return_tasks', $meta, $this->data);
        }
    }
    ...
    public function suggestions()
    {
        ...
        $rows = $this->task_model->getProductNames($sr);
        if ($rows) {
            $r = 0;
            foreach ($rows as $row) {
                ...
                $options              = $this->task_model->getProductOptions($row->id);
                if ($options) {
                    $opt = $option_id && $r == 0 ? $this->task_model->getProductOptionByID($option_id) : current($options);
                    if (!$option_id || $r > 0) {
                        $option_id = $opt->id;
                    }
                } else {
                ...
                $row->unit             = $row->tasks_unit ? $row->tasks_unit : $row->unit;
                $row->new_entry        = 1;
                $row->expiry           = '';
                $row->qty              = 1;
    ...
    public function update_status($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->task_model->updateStatus($id, $status, $note)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'task/update_status', $this->data);
        }
    }

    public function view($tasks_id = null)
    {
        $this->sma->checkPermissions('index');
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('task'), 'page' => lang('task')], ['link' => '#', 'page' => lang('view')]];
        $meta = ['page_title' => lang('view_tasks_details'), 'bc' => $bc];
        $this->page_construct('task/view', $meta, $this->data);
    }
    ...
    public function view_return($id = null)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv                 = $this->task_model->getReturnByID($id);
        if (!$this->session->userdata('view_right')) {
            $this->sma->view_rights($inv->created_by);
        }
        $this->data['barcode']   = "<img src='" . admin_url('products/gen_barcode/' . $inv->reference_no) . "' alt='" . $inv->reference_no . "' class='pull-left' />";
        $this->data['supplier']  = $this->site->getCompanyByID($inv->supplier_id);
        $this->data['payments']  = $this->task_model->getPaymentsForTask($id);
        $this->data['user']      = $this->site->getUser($inv->created_by);
        $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv']       = $inv;
        $this->data['rows']      = $this->task_model->getAllReturnItems($id);
        $this->data['tasks']  = $this->task_model->getTaskByID($inv->tasks_id);
        $this->load->view($this->theme . 'task/view_return', $this->data);
    }
}
