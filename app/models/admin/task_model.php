...
class task_model extends CI_Model
{
    ...
    public function addPayment($data = [])
    {
        if ($this->db->insert('payments', $data)) {
            ...
            $this->site->syncTasksPayments($data['tasks_id']);
            return true;
        }
        return false;
    }
    ...
    public function addTasks($data, $items, $attachments = [])
    {
        ...
        if ($this->db->insert('task', $data)) {
            ...
            if (!empty($attachments)) {
                foreach ($attachments as $attachment) {
                    $attachment['subject_id']   = $tasks_id;
                    $attachment['subject_type'] = 'tasks';
                    $this->db->insert('attachments', $attachment);
                }
            }

            if ($data['status'] == 'returned') {
                $this->db->update('task', ['return_tasks_ref' => $data['return_tasks_ref'], 'surcharge' => $data['surcharge'], 'return_tasks_total' => $data['grand_total'], 'return_id' => $tasks_id], ['id' => $data['tasks_id']]);
            }

            if ($data['status'] == 'received' || $data['status'] == 'returned') {
                $this->site->syncQuantity(null, $tasks_id);
            }
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the sale (Add:task_model.php)');
        } else {
            return true;
        }
        return false;
    ...
    public function calculateTasksTotals($id, $return_id, $surcharge)
    {
        $tasks = $this->getTasksByID($id);
        $items    = $this->getAllTasksItems($id);
        if (!empty($items)) {
            $total            = 0;
            $product_tax      = 0;
            $order_tax        = 0;
            ...
            if ($tasks->order_discount_id) {
                $percentage        = '%';
                $order_discount_id = $tasks->order_discount_id;
            ...
            if ($tasks->order_tax_id) {
                $order_tax_id = $tasks->order_tax_id;
            ...
            $grand_total    = $total          + $total_tax          + $tasks->shipping - $order_discount          + $surcharge;
            $data           = [
                'total'            => $total,
    ...
    public function deletePayment($id)
    {
        ...
        if ($this->db->delete('payments', ['id' => $id])) {
            $this->site->syncTasksPayments($opay->tasks_id);
            return true;
        }
        return false;
    }
    ...
    public function deleteTasks($id)
    {
        $this->db->trans_start();
        $tasks       = $this->getTasksByID($id);
        $tasks_items = $this->site->getAllTasksItems($id);
        $this->site->log('Tasks', ['model' => $tasks, 'items' => $tasks_items]);
        if ($this->db->delete('tasks_items', ['tasks_id' => $id]) && $this->db->delete('task', ['id' => $id])) {
            $this->db->delete('payments', ['tasks_id' => $id]);
            if ($tasks->status == 'received' || $tasks->status == 'partial') {
                foreach ($tasks_items as $oitem) {
                    ...
                    if ($oitem->quantity_balance < $received) {
                        $clause = ['tasks_id' => null, 'transfer_id' => null, 'product_id' => $oitem->product_id, 'warehouse_id' => $oitem->warehouse_id, 'option_id' => $oitem->option_id];
                        $this->site->setTasksItem($clause, ($oitem->quantity_balance - $received));
                    }
                }
            }
            $this->db->delete('attachments', ['subject_id' => $id, 'subject_type' => 'tasks']);
            $this->site->syncQuantity(null, null, $tasks_items);
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the sale (Delete:task_model.php)');
        } else {
            return true;
        }
        return false;
    ...
    public function getAllTasksItems($tasks_id)
    {
        $this->db->select('tasks_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code, products.second_name as second_name')
            ->join('products', 'products.id=tasks_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=tasks_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=tasks_items.tax_rate_id', 'left')
            ->group_by('tasks_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('tasks_items', ['tasks_id' => $tasks_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->select('return_tasks_items.*, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code, products.second_name as second_name')
            ->join('products', 'products.id=return_tasks_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=return_tasks_items.option_id', 'left')
            ->group_by('return_tasks_items.id')
            ->order_by('id', 'asc');
        $q = $this->db->get_where('return_tasks_items', ['return_id' => $return_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $q = $this->db->get_where('tasks_items', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getPaymentsForTasks($tasks_id)
    {
        ...
        $q = $this->db->get_where('payments', ['tasks_id' => $tasks_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $q = $this->db->get_where('task', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getTasksPayments($tasks_id)
    {
    ...
    {
        $q = $this->db->get_where('return_task', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function returnTasks($data = [], $items = [])
    {
        $tasks_items = $this->site->getAllTasksItems($data['tasks_id']);

        if ($this->db->insert('return_task', $data)) {
            ...
            foreach ($items as $item) {
                ...
                $this->db->insert('return_tasks_items', $item);

                if ($tasks_item = $this->getPurcahseItemByID($item['tasks_item_id'])) {
                    if ($tasks_item->quantity == $item['quantity']) {
                        $this->db->delete('tasks_items', ['id' => $item['tasks_item_id']]);
                    } else {
                        $nqty          = $tasks_item->quantity          - $item['quantity'];
                        $bqty          = $tasks_item->quantity_balance  - $item['quantity'];
                        $rqty          = $tasks_item->quantity_received - $item['quantity'];
                        $tax           = $tasks_item->unit_cost         - $tasks_item->net_unit_cost;
                        $discount      = $tasks_item->item_discount / $tasks_item->quantity;
                        $item_tax      = $tax                      * $nqty;
                        $item_discount = $discount                 * $nqty;
                        $subtotal      = $tasks_item->unit_cost * $nqty;
                        $this->db->update('tasks_items', ['quantity' => $nqty, 'quantity_balance' => $bqty, 'quantity_received' => $rqty, 'item_tax' => $item_tax, 'item_discount' => $item_discount, 'subtotal' => $subtotal], ['id' => $item['tasks_item_id']]);
                    }
                }
            }
            $this->calculateTasksTotals($data['tasks_id'], $return_id, $data['surcharge']);
            $this->site->syncQuantity(null, null, $tasks_items);
            $this->site->syncQuantity(null, $data['tasks_id']);
            return true;
        }
        return false;
    }
    ...
    public function updateTasks($id, $data, $items = [], $attachments = [])
    {
        $this->db->trans_start();
        $otasks = $this->getTasksByID($id);
        $oitems    = $this->getAllTasksItems($id);
        if ($this->db->update('task', $data, ['id' => $id]) && $this->db->delete('tasks_items', ['tasks_id' => $id])) {
            $tasks_id = $id;
            foreach ($items as $item) {
                $item['tasks_id'] = $id;
                $item['option_id']   = !empty($item['option_id']) && is_numeric($item['option_id']) ? $item['option_id'] : null;
            ...
            $this->site->syncTasksPayments($id);
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the sale (Update:task_model.php)');
        } else {
            return true;
        }
    ...
    public function updateStatus($id, $status, $note)
    {
        $this->db->trans_start();
        $tasks = $this->getTasksByID($id);
        $items    = $this->site->getAllTasksItems($id);
        if ($this->db->update('task', ['status' => $status, 'note' => $note], ['id' => $id])) {
            if (($tasks->status != 'received' || $tasks->status != 'partial') && ($status == 'received' || $status == 'partial')) {
                foreach ($items as $item) {
                    ...
                    $this->db->update('tasks_items', ['status' => $status, 'quantity_balance' => $qb, 'quantity_received' => $qr], ['id' => $item->id]);
                    $this->updateAVCO(['product_id' => $item->product_id, 'warehouse_id' => $item->warehouse_id, 'quantity' => $item->quantity, 'cost' => $item->real_unit_cost]);
                }
                $this->site->syncQuantity(null, null, $items);
            } elseif (($tasks->status == 'received' || $tasks->status == 'partial') && ($status == 'ordered' || $status == 'pending')) {
                foreach ($items as $item) {
                    $qb = 0;
                    $qr = 0;
                    $this->db->update('tasks_items', ['status' => $status, 'quantity_balance' => $qb, 'quantity_received' => $qr], ['id' => $item->id]);
        ...
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the sale (UpdateStatus:task_model.php)');
        } else {
            return true;
        }
        return false;
