    ...
    $(document).on('click', '#delete-attachment', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    _this.parent('.list-group-item').remove();
                }
            ...
            error: function (data) {
                bootbox.alert('Ajax call failed');
            },
        });
        return false;
    });
    ...
    e('.dropmenu').click(function (t) {
        ...
        if (e('#sidebar-left').hasClass('minified')) {
            if (!e(this).hasClass('open')) {
                ...
                e(this).find('.chevron').hasClass('closed')
                    ? e(this).find('.chevron').removeClass('closed').addClass('opened')
...
$(document).ready(function () {
    cssStyle();
    $('select, .select').not('.skip').select2({ minimumResultsForSearch: 7 });
...
$(document).ready(function () {
    ...
    $(document).on('click', '#set_avg_cost', function (e) {
        e.preventDefault();
        $('#form_action').val($(this).attr('data-action'));
        $('#action-form-submit').trigger('click');
    });
...
$(document).ready(function () {
    ...
    $(document).on('click', '.po', function (e) {
        e.preventDefault();
        var place = site.settings.rtl == 1 ? 'right' : 'left';
        $('.po').popover({ html: true, placement: place, trigger: 'manual' }).popover('show').not(this).popover('hide');
        return false;
    });
    ...
    $('#daterange').daterangepicker(
        {
            ...
            ranges: {
                ...
                Yesterday: [
                    moment().subtract('days', 1).hours(0).minutes(0).seconds(0),
                    moment().subtract('days', 1).hours(23).minutes(59).seconds(59),
                ],
                ...
                'This Month': [
                    moment().startOf('month').hours(0).minutes(0).seconds(0),
                    moment().endOf('month').hours(23).minutes(59).seconds(59),
                ],
                'Last Month': [
                    moment().subtract('month', 1).startOf('month').hours(0).minutes(0).seconds(0),
                    moment().subtract('month', 1).endOf('month').hours(23).minutes(59).seconds(59),
                ],
            },
        },
        function (start, end) {
...
function formatQuantity(x) {
    return x != null
        ? '<div class="text-center">' + formatNumber(x, site.settings.qty_decimals) + '</div>'
...
$(document).ready(function () {
    $('#view-customer').click(function (e) {
        e.preventDefault();
    ...
    $('#view-supplier').click(function (e) {
        e.preventDefault();
...
if (site.settings.auto_detect_barcode == 1) {
    $(document).ready(function () {
        ...
        $(window).keypress(function (e) {
            // if (e.target.id != 'add_item' && e.target.tagName != 'INPUT') {
...
function parse_scale_barcode(barcode) {
    if (barcode.length != site.settings.ws_barcode_chars) {
        return barcode;
    }
    var price = false;
    var weight = false;
    var item_code = barcode.substr(site.settings.item_code_start - 1, site.settings.item_code_chars);
    if (site.settings.ws_barcode_type == 'price') {
        try {
            price = barcode.substr(site.settings.price_start - 1, site.settings.price_chars);
            price = site.settings.price_divide_by ? price / site.settings.price_divide_by : price;
        } catch (err) {
            price = 0;
        }
    } else {
        try {
            weight = barcode.substr(site.settings.weight_start - 1, site.settings.weight_chars);
            weight = site.settings.weight_divide_by ? weight / site.settings.weight_divide_by : weight;
        } catch (err) {
            weight = 0;
        }
    }
