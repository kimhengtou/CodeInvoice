...
<script type="text/javascript">
    ...
    ?>
        ...
        $("#add_item").autocomplete({
            source: '<?= admin_url('task/suggestions'); ?>',
            minLength: 1,
            autoFocus: false,
            delay: 250,
            ...
            select: function (event, ui) {
                ...
                if (ui.item.id !== 0) {
                    var row = add_tasks_item(ui.item);
                    if (row)
                        $(this).val('');
                } else {
        ...
        $(document).on('click', '#addItemManually', function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.msg == 'success') {
                        row = add_tasks_item(data.result);
                    } else {
                        msg = data.msg;
                    }
                }
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('edit_tasks'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart('task/edit/' . $inv->id, $attrib)
                ?>
                    ...
                    ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang('date', 'podate'); ?>
                                    <?php echo form_input('date', ($_POST['date'] ?? $this->sma->hrld($tasks->date)), 'class="form-control input-tip datetime" id="podate" required="required"'); ?>
                                </div>
                            </div>
                        <?php
                } ?>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', ($_POST['reference_no'] ?? $tasks->reference_no), 'class="form-control input-tip" id="poref" required="required"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                echo form_dropdown('warehouse', $wh, ($_POST['warehouse'] ?? $tasks->warehouse_id), 'id="powarehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('warehouse') . '" required="required" style="width:100%;" ');
                                ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                echo form_dropdown('status', $post, ($_POST['status'] ?? $tasks->status), 'id="postatus" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('status') . '" required="required" style="width:100%;" ');
                                ?>
                            </div>
                        </div>
