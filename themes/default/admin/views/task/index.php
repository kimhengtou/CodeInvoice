...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#POData').dataTable({
            ...
            'sAjaxSource': '<?=admin_url('task/getTask' . ($warehouse_id ? '/' . $warehouse_id : ''))?>',
            ...
            {column_number: 4, filter_default_label: "[<?=lang('tasks_status');?>]", filter_type: "text", data: []},
...
<?php if ($Owner || ($GP && $GP['bulk_actions'])) {
            echo admin_form_open('task/tasks_actions', 'id="action-form"');
}
?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('task') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?=admin_url('task/add')?>">
                                <i class="fa fa-plus-circle"></i> <?=lang('add_tasks')?>
                            </a>
                        </li>
                        <li>
                        ...
                        <li class="divider"></li>
                        <li>
                            <a href="#" class="bpo" title="<b><?=lang('delete_task')?></b>"
                                data-content="<p><?=lang('r_u_sure')?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?=lang('i_m_sure')?></a> <button class='btn bpo-close'><?=lang('no')?></button>"
                                data-html="true" data-placement="left">
                                <i class="fa fa-trash-o"></i> <?=lang('delete_task')?>
                            </a>
                        </li>
                    </ul>
                </li>
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=admin_url('task')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($warehouses as $warehouse) {
                                echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . admin_url('task/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            } ?>
                        </ul>
                    </li>
                    <?php
            ...
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="POData" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <thead>
                        <tr class="active">
                            <th style="min-width:20px; width: 30px; text-align: center;">
                                <input class="checkbox checkft" type="checkbox" name="check"/>
                            </th>
                            <!-- <th><?= lang('date'); ?></th>
                            <th><?= lang('ref_no'); ?></th>
                            <th><?= lang('supplier'); ?></th>
                            <th><?= lang('tasks_status'); ?></th>
