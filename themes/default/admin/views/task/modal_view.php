...
<div class="modal-dialog modal-lg no-modal-header">
    ...
    ?>
        ...
        echo lang('return_ref') . ': ' . $inv->return_tasks_ref;
        if ($inv->return_id) {
                ...
                <table class="table table-bordered table-hover table-striped print-table order-table">
                    ...
                    <?php if ($inv->grand_total != $inv->total) {
                        ...
                        <tr>
                            ...
                            <?php
                            if ($Settings->tax1 && $inv->product_tax > 0) {
                                echo '<td class="text-right">' . $this->sma->formatMoney($return_tasks ? ($inv->product_tax + $return_tasks->product_tax) : $inv->product_tax) . '</td>';
                            }
                        if ($Settings->product_discount && $inv->product_discount != 0) {
                            echo '<td class="text-right">' . $this->sma->formatMoney($return_tasks ? ($inv->product_discount + $return_tasks->product_discount) : $inv->product_discount) . '</td>';
                        } ?>
                            <td class="text-right"><?= $this->sma->formatMoney($return_tasks ? (($inv->total + $inv->product_tax) + ($return_tasks->total + $return_tasks->product_tax)) : ($inv->total + $inv->product_tax)); ?></td>
                        </tr>
                    <?php
                    } ?>
                    <?php
                    if ($return_tasks) {
                        echo '<tr><td colspan="' . $col . '" class="text-right">' . lang('return_total') . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->sma->formatMoney($return_tasks->grand_total) . '</td></tr>';
                    }
                    if ($inv->surcharge != 0) {
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" class="text-right">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td class="text-right">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->sma->formatMoney($return_tasks ? ($inv->order_discount + $return_tasks->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr><td colspan="' . $col . '" class="text-right">' . lang('order_tax') . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->sma->formatMoney($return_tasks ? ($inv->order_tax + $return_tasks->order_tax) : $inv->order_tax) . '</td></tr>';
                    }
                    ?>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right; font-weight:bold;"><?= $this->sma->formatMoney($return_tasks ? ($inv->grand_total + $return_tasks->grand_total) : $inv->grand_total); ?></td>
                    </tr>
                    <tr>
                        <td colspan="<?= $col; ?>"
            ...
            <?php if (!$Supplier || !$Customer) {
                ...
                <div class="buttons">
                    <div class="btn-group btn-group-justified">
                        <div class="btn-group">
                            <a href="<?= admin_url('task/add_payment/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal2" class="tip btn btn-primary" title="<?= lang('add_payment') ?>">
                                <i class="fa fa-dollar"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('add_payment') ?></span>
                            </a>
                        </div>
                        <div class="btn-group">
                            <a href="<?= admin_url('task/email/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal2" class="tip btn btn-primary" title="<?= lang('email') ?>">
                        ...
                        <div class="btn-group">
                            <a href="<?= admin_url('task/edit/' . $inv->id) ?>" class="tip btn btn-warning sledit" title="<?= lang('edit') ?>">
