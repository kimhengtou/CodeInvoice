...
<head>
    ...
    <title><?= $this->lang->line('tasks') . ' ' . $inv->reference_no; ?></title>
                ...
                ?>
                    ...
                    <?php if ($inv->grand_total != $inv->total) {
                        ...
                        if ($Settings->product_discount && $inv->product_discount != 0) {
                            ...
                            <td style="text-align:right;"><?= $this->sma->formatMoney($return_tasks ? (($inv->total + $inv->product_tax) + ($return_tasks->total + $return_tasks->product_tax)) : ($inv->total + $inv->product_tax)); ?></td>
                        </tr>
                    <?php
                    } ?>
                    <?php
                    if ($return_tasks) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang('return_total') . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_tasks->grand_total) . '</td></tr>';
                    }
                    if ($inv->surcharge != 0) {
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->sma->formatMoney($return_tasks ? ($inv->order_discount + $return_tasks->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang('order_tax') . ' (' . $default_currency->code . ')</td><td style="text-align:right;">' . $this->sma->formatMoney($return_tasks ? ($inv->order_tax + $return_tasks->order_tax) : $inv->order_tax) . '</td></tr>';
                    }
                    ?>
