...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-file"></i><?= lang('tasks_no') . '. ' . $inv->id; ?></h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?= admin_url('task/payments/' . $inv->id) ?>" data-target="#myModal" data-toggle="modal">
                                <i class="fa fa-money"></i> <?= lang('view_payments') ?>
                            </a>
                        </li>
                        <li>
                            <a href="<?= admin_url('task/add_payment/' . $inv->id) ?>" data-target="#myModal" data-toggle="modal">
                                <i class="fa fa-money"></i> <?= lang('add_payment') ?>
                            </a>
                        </li>
                        <li>
                            <a href="<?= admin_url('task/edit/' . $inv->id) ?>">
                                <i class="fa fa-edit"></i> <?= lang('edit_tasks') ?>
                            </a>
                        </li>
                        <li>
                            <a href="<?= admin_url('task/email/' . $inv->id) ?>">
                                <i class="fa fa-envelope-o"></i> <?= lang('send_email') ?>
                            </a>
                        </li>
                        <li>
                            <a href="<?= admin_url('task/pdf/' . $inv->id) ?>">
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                <?php if (!empty($inv->return_tasks_ref) && $inv->return_id) {
    echo '<div class="alert alert-info no-print"><p>' . lang('tasks_is_returned') . ': ' . $inv->return_tasks_ref;
    echo ' <a data-target="#myModal2" data-toggle="modal" href="' . admin_url('task/modal_view/' . $inv->return_id) . '"><i class="fa fa-external-link no-print"></i></a><br>';
    echo '</p></div>';
} ?>
                ...
                <div class="well well-sm">
                    ...
                    <div class="col-xs-4 border-left">
                        ...
                        <div class="col-xs-10">
                            ...
                            <?php if (!empty($inv->return_tasks_ref)) {
                                echo '<p>' . lang('return_ref') . ': ' . $inv->return_tasks_ref;
                                if ($inv->return_id) {
                                    echo ' <a data-target="#myModal2" data-toggle="modal" href="' . admin_url('task/modal_view/' . $inv->return_id) . '"><i class="fa fa-external-link no-print"></i></a><br>';
                                } else {
                                    echo '</p>';
                            ...
                            if ($inv->payment_status != 'paid' && $inv->due_date) {
                                echo '<p style="font-weight:bold;">' . lang('due_date') . ': ' . $this->sma->hrsd($inv->due_date) . '</p>';
                            } ?>
                        </div>
                        <div class="col-xs-12 order_barcodes">
                            ...
                            <?= $this->sma->qrcode('link', urlencode(admin_url('task/view/' . $inv->id)), 2); ?>
                        </div>
                        <div class="clearfix"></div>
                    ...
                    <table class="table table-bordered table-hover table-striped print-table order-table">
                        ...
                        <?php if ($inv->grand_total != $inv->total) {
                            ...
                            <tr>
                                ...
                                <?php
                                if ($Settings->tax1 && $inv->product_tax > 0) {
                                    echo '<td style="text-align:right;">' . $this->sma->formatMoney($return_tasks ? ($inv->product_tax + $return_tasks->product_tax) : $inv->product_tax) . '</td>';
                                }
                            if ($Settings->product_discount && $inv->product_discount != 0) {
                                echo '<td style="text-align:right;">' . $this->sma->formatMoney($return_tasks ? ($inv->product_discount + $return_tasks->product_discount) : $inv->product_discount) . '</td>';
                            } ?>
                                <td style="text-align:right; padding-right:10px;"><?= $this->sma->formatMoney($return_tasks ? (($inv->total + $inv->product_tax) + ($return_tasks->total + $return_tasks->product_tax)) : ($inv->total + $inv->product_tax)); ?></td>
                            </tr>
                        <?php
                        } ?>
                        <?php
                        if ($return_tasks) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang('return_total') . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->sma->formatMoney($return_tasks->grand_total) . '</td></tr>';
                        }
                        if ($inv->surcharge != 0) {
                        ...
                        <?php if ($Settings->indian_gst) {
                            if ($inv->cgst > 0) {
                                $cgst = $return_tasks ? $inv->cgst + $return_tasks->cgst : $inv->cgst;
                            ...
                            if ($inv->sgst > 0) {
                                $sgst = $return_tasks ? $inv->sgst + $return_tasks->sgst : $inv->sgst;
                            ...
                            if ($inv->igst > 0) {
                                $igst = $return_tasks ? $inv->igst + $return_tasks->igst : $inv->igst;
                        ...
                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang('order_discount') . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->sma->formatMoney($return_tasks ? ($inv->order_discount + $return_tasks->order_discount) : $inv->order_discount) . '</td></tr>';
                        }
                        ?>
                        <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;">' . lang('order_tax') . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->sma->formatMoney($return_tasks ? ($inv->order_tax + $return_tasks->order_tax) : $inv->order_tax) . '</td></tr>';
                        }
                        ?>
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; padding-right:10px; font-weight:bold;"><?= $this->sma->formatMoney($return_tasks ? ($inv->grand_total + $return_tasks->grand_total) : $inv->grand_total); ?></td>
                        </tr>
                        <tr>
                            <td colspan="<?= $col; ?>"
                            ...
                            <td style="text-align:right; font-weight:bold;"><?= $this->sma->formatMoney($return_tasks ? ($inv->paid + $return_tasks->paid) : $inv->paid); ?></td>
                        </tr>
                        <tr>
                            <td colspan="<?= $col; ?>"
                            ...
                            <td style="text-align:right; font-weight:bold;"><?= $this->sma->formatMoney(($return_tasks ? ($inv->grand_total + $return_tasks->grand_total) : $inv->grand_total) - ($return_tasks ? ($inv->paid + $return_tasks->paid) : $inv->paid)); ?></td>
                        </tr>
                ...
                <div class="row">
                    <div class="col-xs-7">
                    <?= $Settings->invoice_view > 0 ? $this->gst->summary($rows, $return_rows, ($return_tasks ? $inv->product_tax + $return_tasks->product_tax : $inv->product_tax), true) : ''; ?>
        ...
        <?php if (!$Supplier || !$Customer) {
            ...
            <div class="buttons">
                <div class="btn-group btn-group-justified">
                    <div class="btn-group">
                        <a href="<?= admin_url('task/payments/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn btn-primary tip" title="<?= lang('view_payments') ?>">
                            <i class="fa fa-money"></i> <span class="hidden-sm hidden-xs"><?= lang('view_payments') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('task/add_payment/' . $inv->id) ?>" class="tip btn btn-primary tip" title="<?= lang('add_payment') ?>" data-target="#myModal" data-toggle="modal">
                            <i class="fa fa-money"></i> <span class="hidden-sm hidden-xs"><?= lang('add_payment') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('task/email/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal" class="tip btn btn-primary tip" title="<?= lang('email') ?>">
                            <i class="fa fa-envelope-o"></i> <span class="hidden-sm hidden-xs"><?= lang('email') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('task/pdf/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('download_pdf') ?>">
                            <i class="fa fa-download"></i> <span class="hidden-sm hidden-xs"><?= lang('pdf') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="<?= admin_url('task/edit/' . $inv->id) ?>" class="tip btn btn-warning tip" title="<?= lang('edit') ?>">
                            <i class="fa fa-edit"></i> <span class="hidden-sm hidden-xs"><?= lang('edit') ?></span>
                        </a>
                    </div>
                    <div class="btn-group">
                        <a href="#" class="tip btn btn-danger bpo" title="<b><?= $this->lang->line('delete_tasks') ?></b>"
                           data-content="<div style='width:150px;'><p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' href='<?= admin_url('task/delete/' . $inv->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button></div>"
